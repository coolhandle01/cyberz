...
@pytest.fixture()
def recon_file(tmp_path: Path, recon_result) -> Iterator[str]:
...
class TestReconSubdomains:
    def test_returns_all_subdomains(self, recon_file: str) -> None:
        result = query.recon_subdomains(recon_file)
        assert result == ["api.example.com", "blog.example.com", "admin.example.com"]

    def test_filters_case_insensitively(self, recon_file: str) -> None:
        assert query.recon_subdomains(recon_file, host_filter="API") == ["api.example.com"]
...
class TestReconEndpoints:
    def test_no_filters_returns_all(self, recon_file: str) -> None:
    ...
    def test_filter_by_status(self, recon_file: str) -> None:
    ...
    def test_conjunctive_filters(self, recon_file: str) -> None:
...
class TestReconOpenPorts:
    def test_returns_all_hosts(self, recon_file: str) -> None:
    ...
    def test_returns_single_host(self, recon_file: str) -> None:
        assert query.recon_open_ports(recon_file, host="redis.example.com") == {
            "redis.example.com": [6379]
        }

    def test_unknown_host_returns_empty(self, recon_file: str) -> None:
        assert query.recon_open_ports(recon_file, host="ghost.example.com") == {}
